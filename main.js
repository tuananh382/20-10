...
console.log(mouse.x,mouse.y)
    var cooX = window.innerWidth/2;
    var cooY = window.innerHeight;
    console.log(cooX, cooY)
	var kc=Math.sqrt((mouse.x - cooX) * (mouse.x - cooX) + (mouse.y - cooY) * (mouse.y - cooY))/50;
...
function initializeVariables() {
  cannon = new Cannon(canvas.width / 2, canvas.height, 40, 10, "white");
  cannonballs = [];
  explosions = [];
  colors = [
